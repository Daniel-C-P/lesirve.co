   ...
   ==========================*/
    ...
    qtyDecs.forEach((qtyDec) => {
        qtyDec.addEventListener("click", function (e) {
            const productoCarrito = e.target.nextElementSibling;
            if (productoCarrito.value > 0) {
                const cantidad = parseInt($(productoCarrito).val()) - 1;
                const productoId = $(productoCarrito).attr("id-producto");
                $.ajax({
                    url: "/editar-cantidad-producto",
                    method: "POST",
                    headers: {
                    ...
                    data: {
                        id_cliente: $('meta[name="id-cliente"]').attr(
                            "content"
                        ),
                        id_producto: productoId,
                        cantidad: cantidad,
                    },
                    success: function (nuevoValor) {
                        $(".total-carrito").html(`$ ${nuevoValor}`);
                        $("#inputCantidad").val(cantidad);
                        $(productoCarrito).val(cantidad);
                    },
                    error: function (msg) {
                        console.log({ msg });
                    },
    ...
    qtyIncs.forEach((qtyDec) => {
        qtyDec.addEventListener("click", function (e) {
            const productoCarrito = e.target.previousElementSibling;
            const cantidad = parseInt($(productoCarrito).val()) + 1;
            const productoId = $(productoCarrito).attr("id-producto");
            $.ajax({
                url: "/editar-cantidad-producto",
                ...
                data: {
                    id_cliente: $('meta[name="id-cliente"]').attr("content"),
                    id_producto: productoId,
                    cantidad: cantidad,
                },
                success: function (nuevoValor) {
   ...
   ==========================*/
    $(".add-cartnoty").on("click", function () {
        $.ajax({
            ...
            data: {
                id_cliente: $('meta[name="id-cliente"]').attr("content"),
                id_producto: $(this).attr("id-producto"),
            },
  ...
  $.ajax({
    ...
    error: function ({ responseJSON }) {
      ...
      (Object.keys(errors)).forEach(error => {
        ...
        $(`#e-${error}`).html(`<span>
          <strong>${errors[error][0]}</strong>
        </span>`);
        $(`#e-${error}`).css('display', 'block');
        if (errors[error].length > 1) {
          $(`#e-${error}_confirmation`).html(`<span>
            <strong>${errors[error][1]}</strong>
          </span>`);
          $(`#e-${error}_confirmation`).css('display', 'block');
